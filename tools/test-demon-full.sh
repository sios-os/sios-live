#!/bin/bash
# test-demon-full.sh — Test DEMON panel in Godot with live daemon
...
timeout 60 xvfb-run --auto-servernum --server-args='-screen 0 1920x1080x24' \
    godot --rendering-driver opengl3 2>&1 | grep -E "Screenshot|RoomController|IPC|connected|DEMON|ERROR|SCRIPT_ERROR" | head -20
