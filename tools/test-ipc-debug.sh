#!/bin/bash
# test-ipc-debug.sh — Debug IPC connection from Godot to daemon.
...
timeout 30 xvfb-run --auto-servernum --server-args='-screen 0 1920x1080x24' \
    godot --rendering-driver opengl3 2>&1 | grep -E "RoomController|Screenshot|ERROR|SCRIPT|IPC|connected" | head -20
